...
def load_tensor(device: torch.device, path: str, *subpaths) -> Optional[torch.Tensor]:
    ...
    if os.path.exists(filename):
        ...
        print(f"the requested file: {filename} not exists!\n")
    return tensor
...
def load_data(
    ...
    """Read data in config.workspace / config.dataset / config.input_directory
    ...
    input_dir = os.path.join(config.workspace, config.dataset, config.input_directory)
    node_filename = config.node_filename
    edge_filename = config.edge_filename

    graph = Graph.read_from_files(
        nodes_filename=os.path.join(input_dir, node_filename),
        edges_filename=os.path.join(input_dir, edge_filename),
    )
    ...
    # print("pairwise_node_features.shape = ", pairwise_node_features.shape)
    ...
    if trajectories.pairwise_node_distances is None:
        print("There is no information for pairwise_node_distances! Target2accuracy can not be computed!\n")
    ...
    print("number of train trajectories: {}, number of test trajectories: {} ".format(len(train_trajectories), len(test_trajectories)))

    return (graph, trajectories, pairwise_node_features)
...
def create_optimizer(params, config: Config) -> torch.optim.Optimizer:
    ...
    if config.optimizer == "SGD":
        ...
        optimizer = AdaHessian(params, lr=config.lr)
    else:
...
def train_epoch(
    ...
    for iteration, batch_start in enumerate(
        ...
        if config.optimizer == "AdaHessian":
            create_graph = type(optimizer).__name__ == "AdaHessian"
...
def main():
    ...
    graph, trajectories, pairwise_node_features = load_data(config)
    print("Data are loaded! \n")
    pairwise_node_features = None       # I don't use pairwise_node_features as extra information
    ...
    print(f'==== START "{config.name}" EXPERIMENT ====')

    torch.manual_seed(config.seed)

    experiment_dir = os.path.join(config.workspace, config.checkpoint_directory, config.name, config.experiment_name)
    if os.path.exists(experiment_dir):
        print("The experiment is already done - please give a different name in config.experiment_name")
        exit()
    else:
    ...
    if config.enable_checkpointing:
        ...
        print(f"Checkpoints will be saved in [{experiment_dir}]")
    ...
    for epoch in range(config.number_epoch):
        ...
        print(f"\n=== EPOCH {epoch} ===")
        with open(os.path.join(experiment_dir, 'results.txt'), 'a') as f:
            f.write(f"\n=== EPOCH {epoch} ===\n\n")
        ...
        if config.enable_checkpointing and epoch % config.chechpoint_every_num_epoch == 0:
            ...
            chkpt_file = os.path.join(experiment_dir, f"{epoch:04d}.pt")
            torch.save(
                {
                    "epoch": epoch,
            ...
            config_file = os.path.join(experiment_dir, "config")
            config.save_to_file(config_file)

            metrics_file = os.path.join(experiment_dir, f"{epoch:04d}.txt")
