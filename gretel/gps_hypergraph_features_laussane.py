...
input_dir = '../workspace/gps/laussane'

"""
Load data
"""
laussane_edge_features = {}
with open(os.path.join(input_dir, "edges.txt")) as f:
    ...
    for line in f:
        ...
        laussane_edge_features[edge_id] = {}
        laussane_edge_features[edge_id]['out_id'] = out_id
        laussane_edge_features[edge_id]['in_id'] = in_id
        laussane_edge_features[edge_id]['feat1'] = feat1
        laussane_edge_features[edge_id]['feat2'] = feat2

laussane_node_features = {}
with open(os.path.join(input_dir, "nodes.txt"), 'r') as f:
    ...
    for i, line in enumerate(f.readlines()):
        ...
        laussane_node_features[node_id] = {}
        laussane_node_features[node_id]['id'] = node_id
        laussane_node_features[node_id]['feat1'] = feat1
        laussane_node_features[node_id]['feat2'] = feat2

gps_edge_ids = {(laussane_edge_features[i]['out_id'], laussane_edge_features[i]['in_id']): i
                for i in range(0, len((laussane_edge_features)))}
n_nodes = 18170
n_edges = 32490
...
with open(os.path.join(input_dir, 'laussane_hyperedge_similarity.txt'), 'w') as f:
    f.write("{}\t{}\n".format(n_edges, 3))
    for i in range(0, n_edges):
        feat = []
        fr = laussane_edge_features[i]['out_id']
        to = laussane_edge_features[i]['in_id']
        ...
        feat.append(fr)
        feat.append(to)
        feat.append(laussane_edge_features[i]['feat1'])
        feat.append(laussane_edge_features[i]['feat2'])
        feat.append(sim[0])
        laussane_edge_features[i]['hyper_similarity'] = sim[0]
...
with open(os.path.join(input_dir, 'laussane_hyperedge_DNnode_in_out_degree.txt'), 'w') as f:
    f.write("{}\t{}\n".format(n_edges, 4))
    for i in range(0, n_edges):
        feat = []
        laussane_edge_features[i]['in_hyper_degree'] = in_degree[i]
        laussane_edge_features[i]['out_hyper_degree'] = out_degree[i]
        feat.append(laussane_edge_features[i]['out_id'])
        feat.append(laussane_edge_features[i]['in_id'])
        feat.append(laussane_edge_features[i]['feat1'])
        feat.append(laussane_edge_features[i]['feat2'])
        feat.append(laussane_edge_features[i]['in_hyper_degree'])
        feat.append(laussane_edge_features[i]['out_hyper_degree'])
        line = str(i) + "\t" + "\t".join(map(str, [f for f in feat])) + "\n"
        f.write(line)


with open(os.path.join(input_dir, 'laussane_hyperedge_similarity_DNnode_in_out_degree.txt'), 'w') as f:
    f.write("{}\t{}\n".format(n_edges, 5))
    for i in range(0, n_edges):
        feat = []
        feat.append(laussane_edge_features[i]['out_id'])
        ...
        feat.append(laussane_edge_features[i]['feat1'])
        feat.append(laussane_edge_features[i]['feat2'])
        feat.append(laussane_edge_features[i]['hyper_similarity'])
        feat.append(laussane_edge_features[i]['in_hyper_degree'])
        feat.append(laussane_edge_features[i]['out_hyper_degree'])
        line = str(i) + "\t" + "\t".join(map(str, [f for f in feat])) + "\n"
        f.write(line)
...
"""
with open(os.path.join(input_dir, 'laussane_hyper_node_features.pickle'), 'wb') as f:
    pickle.dump(laussane_node_features, f, protocol=pickle.HIGHEST_PROTOCOL)

with open(os.path.join(input_dir, 'laussane_hyper_edge_features.pickle'), 'wb') as f:
    pickle.dump(laussane_edge_features, f, protocol=pickle.HIGHEST_PROTOCOL)


# load nodes features
# with open(os.path.join(input_dir, 'laussane_hyper_node_features.pickle'), 'rb') as f:
#     laussane_gps_node_features = pickle.load(f)


# load edge features
# with open(os.path.join(input_dir, 'laussane_hyper_edge_features.pickle'), 'rb') as f:
#     laussane_gps_edge_features = pickle.load(f)
