...
class Evaluator:
    ...
    def update_metrics(
        ...
        for pred_id in range(n_pred):
            ...
            if not rw_non_backtracking:
                ...
                for i, (s, r) in enumerate(zip(traversed_edges[:-1], traversed_edges[1:])):
                    ...
                    if nb_rw_graph.edge(s, r).nelement() == 0:
                        print("s {} r {} s >= r {}".format(s, r, s >= r))
                        traversed_edges_weights[i + 1] = 0
                    else:
                        traversed_edges_weights[i + 1] = nb_rw_graph.edge(s, r)
        ...
        if self.config.dataset == "wikispeedia":
            ...
            # In lukas' version it was target_ranks. I changed it in target_ranks.long() because I had error message
            self.update_metrics_by_keys(
                "target_rank", jump_lengths, self.harmonic_numbers[target_ranks.long() - 1]
            )

            """West target accuracy"""
            start_nodes = observations[starts].nonzero()[:, 1]
            if trajectories.pairwise_node_distances is None:
                target2_acc = torch.zeros(1)
            else:
...
def target2_accuracy(
    ...
    #print("possible_targets_mask.dtype = ", possible_targets_mask.dtype)
    # n_pred
    ...
    # In lukas' version it was possible_targets_mask. I changed it in possible_targets_mask.bool() because I had error message
    correctly_classified &= possible_targets_mask.bool()
